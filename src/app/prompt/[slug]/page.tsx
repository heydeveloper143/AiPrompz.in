  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!snap.empty) {
          const data = snap.docs[0].data() as Prompt;
        ...
        const trending: Prompt[] = snap2.docs.map(d => ({ id: d.id, ...d.data() as Prompt }));
        setTrendingPosts(trending.slice(0, 5));

        setLoading(false);

        // Trigger AdSense after data fetch
          ...
          <article className="flex-1 space-y-8">
            ...
            {prompt.imageUrl && (
              <img
                src={prompt.imageUrl}
                alt={prompt.title}
                className="h-96 w-full max-w-full object-contain transition-transform duration-500 hover:scale-105"
              />
            )}

            <pre className="whitespace-pre-wrap text-gray-800 text-sm select-all">
              {prompt.promptText}
            </pre>
